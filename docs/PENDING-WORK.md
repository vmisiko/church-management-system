...
**Status:** ✅ **Done 2026-09-26.** One scope refinement made during implementation: last-seen and engagement score are shown on the member **detail** view only, not the list — computing either per row for a paginated list would add real cost to the list endpoint B9 just tuned, and neither value is meaningful at a glance across 50 rows the way it is for one member you're looking at. `GET /members/:id/engagement` computes both in a single query set (three CTEs: 90-day attendance ratio, follow-up connected-ratio, and involvement, joined with a last-seen `MAX`) — cheap because it only runs for one member, not the whole page.

**Engagement score formula (documented starting point, not signed off):** `round(attendanceRatio * 60 + followUpRatio * 20 + involvement * 20)`, all inputs 0–1. Revisit the weights if this needs to be authoritative rather than illustrative — it's clearly labelled in the API doc comment and this write-up as a starting formula.

**Milestones:** new module (`src/milestones`) — `milestone_types` (admin-editable via `GET/POST/PATCH/DELETE /milestone-types`, `super_admin`/`admin` for mutation, deletion refused with 409 if a member still has that milestone recorded) and `member_milestones` (`GET/POST /members/:id/milestones`, `DELETE /members/:id/milestones/:milestoneId`). Seeded with 3 starter types (Baptism, Discipleship Class Completed, Leadership Training) that can be renamed or deleted — not a hardcoded taxonomy, per the scoping decision. Skipped the per-operation usecase-class layer this codebase otherwise uses for CRUD features (service calls the repository directly) to keep the file count proportionate to a genuinely simple CRUD surface; documented as a deliberate, minor convention deviation.

Frontend: member detail drawer gained an "Engagement" section (last-seen date, score out of 100) and a "Milestones" section (list + inline add form with a type picker and optional date/notes + delete). The milestones feature also skips the per-operation usecase layer on the frontend to match.

**Verified:** backend 56/56 suites, 607/607 tests (12 new tests for the milestones module — service + controller — plus new coverage for the members controller's 4 new endpoints and `MemberRepository.getEngagement`'s scoring math), clean build/lint. All 18 migrations (17 pre-existing + this one) run clean on an empty database. Frontend clean typecheck/lint/build, 6/6 suites, 34/34 tests. Live end-to-end in the browser against the seeded dev database: engagement section showed a real last-seen date and score; recorded a Baptism milestone against a member, confirmed it displayed with the correct date, then deleted it and confirmed it disappeared.
