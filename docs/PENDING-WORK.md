...
**Status:** ✅ **Done 2026-09-25.** Backend: migration `AddZoneOverseer` (nullable FK to `members`, `ON DELETE SET NULL`, mirrors `fellowships.leader_id` exactly); `CreateFellowshipZoneDto`/`UpdateFellowshipZoneDto`/response DTO all carry `overseerId`. The zone `update` endpoint and use case already existed end to end (`PATCH /fellowship-zones/:id`) — just never had a frontend caller. Frontend: `AddZoneDialog` already supported a dual create/edit mode (`zone?` prop, unused until now) — added an overseer picker to it, and added a genuinely new "Zones" section to the fellowships page (there wasn't one; zones only existed as a filter dropdown before) listing each zone's name, its overseer or a "No overseer assigned" warning badge (same `text-warning`/`border-warning` convention used elsewhere in the app), and an Edit button that opens the dialog in edit mode.

**Verified:** backend 54/54 suites, 586/586 tests, clean build/lint; all 17 migrations (16 pre-existing + this one) run clean on an empty database; frontend clean typecheck/lint/build; live end-to-end in the browser against the seeded dev database — assigned "Timothy Barasa" as Eastlands zone's overseer, confirmed the warning badge disappeared and "Overseer: Timothy Barasa" displayed, while other zones correctly still showed the no-overseer warning.
...
**Status:** ✅ **Done 2026-09-26.** Backend: last-seen and a 0–100 engagement score computed on demand via `GET /members/:id/engagement` (not embedded in the paginated member list, to avoid adding per-row join cost to the list endpoint B9 tuned). The score is a **documented starting formula, not signed off**: `round(attendanceRatio * 60 + followUpRatio * 20 + involvement * 20)` (90-day attendance rate, follow-up "connected" rate, department/fellowship involvement) — revisit the weights if this needs to be authoritative. New `milestones` module: `milestone_types` is admin-editable (not a hardcoded enum), seeded with 3 starters (Baptism, Discipleship Class Completed, Leadership Training); `member_milestones` nested under members (`/members/:id/milestones`); deleting a still-referenced type is refused with 409. Frontend: member detail drawer gained an "Engagement" section (last-seen, score) and a "Milestones" section (list, inline add form with type picker + optional date/notes, delete). Both sides skip the per-operation usecase-class layer for this genuinely simple CRUD surface — a documented, minor convention deviation.

**Verified:** backend 56/56 suites, 607/607 tests, clean build/lint; all 18 migrations (17 pre-existing + this one) run clean on an empty database; frontend 6/6 suites, 34/34 tests, clean build/lint; live in the browser against the seeded dev database — engagement section showed a real last-seen date and score, recorded a Baptism milestone, confirmed it displayed, deleted it, confirmed it disappeared. Merged: backend nestapi-cms#20, frontend church-management-system#32.
...
**Status:** ✅ **Done 2026-09-26.** Backend: migration `CreateCareRecords` (`care_record_type` enum: visit/call/hospital/bereavement/financial_need/other; `care_record_status` enum: open/resolved; `care_records` table with `member_id` FK `CASCADE`, `handled_by` FK to `users` `SET NULL`, and indexes on `(member_id, created_at)` and `status`); new `care` module (domain/infrastructure/application/presentation layers, skipping the per-operation usecase-class layer for this genuinely simple CRUD surface, matching C5c's milestones); three endpoints nested under members (`GET/POST /members/:id/care-records`, `PATCH /members/:id/care-records/:careRecordId`) with the same role guards as the follow-ups controller; `resolvedAt` auto-set/cleared on status transitions, mirroring `follow_up_tasks.completedAt`. Frontend: new "Care" section in the member detail drawer — a "Log contact" toggle, inline add form (type select, optional notes), and a history list with status badges and a "Resolve" action, mirroring the follow-up-attempt UI pattern.

**Bug found and fixed during live verification:** the PATCH `/care-records/:id` response immediately after resolving a record was missing its `type` and notes — a fresh page reload showed the data correctly, so it wasn't data loss, just a wrong response. Root cause: `UpdateCareRecordDto`'s optional fields (`type?`, `notes?`) are class fields, and this project's `tsconfig.json` targets ES2023 (`useDefineForClassFields` defaults to true), so an unset optional field exists as an own property with value `undefined` rather than not existing at all. `CareRecordRepository.update()`'s `Object.assign(record, data)` therefore overwrote the in-memory `type`/`notes` with `undefined` whenever a PATCH omitted them (e.g. resolving only sends `{status}`) — TypeORM's `save()` silently skips persisting `undefined` columns, so the database stayed correct, but the object returned to the caller (and thus the frontend's optimistic store update) was wrong. Fixed by filtering out `undefined` entries before merging instead of a blanket `Object.assign`. Added a repository-level regression test (`care-record.repository.spec.ts`) using a real `UpdateCareRecordDto` instance (not a plain object literal) that fails without the fix and passes with it — confirmed by reverting the fix locally and re-running the test before restoring it.

**Verified:** backend 58/58 suites, 621/621 tests (4 new repository-level tests, including the regression test above), clean `tsc`/`nest build`/`eslint`; frontend `tsc`/`pnpm lint`/`vitest run` (6/6 suites, 34/34 tests)/`pnpm build` clean; live end-to-end in the browser against the seeded dev database — logged a care record, confirmed it displayed with the correct type/status/date/notes, resolved it, confirmed the status updated with all fields intact (both immediately and after a fresh reload).
