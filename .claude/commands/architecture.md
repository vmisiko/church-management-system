...
│   │       └── BaseUseCase.ts
│   └── di/
│       └── DependencyLocator.ts  # Controller factory hooks
│
...
```ts
// application/member/useMembersStore.ts
...
```ts
// application/member/useMembersController.ts
"use client"
...
### 3. Domain — use-case(s)
```ts
// domain/<feature>/Get<Feature>sUseCase.ts
...
```ts
// data/mock/<Feature>Repository.ts
...
```ts
// application/<feature>/use<Feature>Controller.ts
import { useCallback } from "react"
import { toast } from "sonner"
...
```tsx
// presentation/components/<feature>/<Feature>Table.tsx
...
| Business logic or use-case calls added directly to a Zustand store | Stores hold state only — orchestration belongs in the controller hook |
