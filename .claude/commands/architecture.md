...
| `onMounted` data fetch | Called on the Ploc instance from `useEffect` or Server Component |
...
Concrete repository implementations. Mock today; swap for `api/` when the NestJS backend is ready — no other layer changes.
...
### `application/` — state stores + Ploc classes (the PloC layer)

Each feature has exactly two files:

- **`use*State.ts`** — Zustand store. State shape only. May use `persist` for localStorage. No logic.
- **`*Ploc.ts`** — class extending `Ploc<StoreApi<State>>`. Receives use-cases via constructor injection. Exposes named methods that orchestrate use-cases, fold results, and update the store. This is the direct equivalent of a Flutter BLoC or Vue `*Ploc`.
...
export class AuthPloc extends Ploc<StoreApi<AuthState>> {
  constructor({
    store,
    loginUseCase,
  }: {
...
**`DependencyLocator.ts`** — wire once, import everywhere:
...
export const membersPloc = new MembersPloc({
  store: useMembersState,   // Zustand's create() return is also a StoreApi
  getMembersUseCase: new GetMembersUseCase(memberRepo),
})
```

**In components** — state from hook, actions from Ploc singleton:
...
- `*Ploc.ts` extends `Ploc<StoreApi<State>>` from `@/core/application/ploc`. Receives use-cases in constructor — never instantiates them.
- All state mutations via `this.store.setState(...)` inside the Ploc.
- Side-effects (`toast`, `router.push`) triggered inside the Ploc, not in components.
- The Ploc singleton comes from `DependencyLocator` — never `new XxxPloc()` inside a component.
...
Thin orchestrators. Server Components may reach into `data/` for initial server-side load.
...
app/login/page.tsx
  └─ <LoginForm /> (presentation, "use client")
       ...
       └─ authPloc.login(creds)     ← action (Ploc singleton)
...
6. Ploc `fold`s: `this.store.setState({ accessToken, loginLoading: false })` + `router.push('/dashboard')`.
7. Zustand notifies `LoginForm`. Component re-renders.
...
```ts
// core/di/DependencyLocator.ts (add to existing exports)
...
export function DepartmentsTable() {
  ...
  useEffect(() => { departmentsPloc.fetchDepartments() }, [])
  // render departments
}
```
...
- Business logic lives in a **UseCase**, not in a Ploc method
- State mutations only via `this.store.setState(...)` inside the Ploc
- The Ploc is imported from `DependencyLocator` — never `new XxxPloc()` in a component
- Errors travel through `Either<DataError, T>`, handled in the Ploc with `fold`
...
| `Either` bypassed with bare `try/catch` in a repository | Data layer must use `Either` consistently |
