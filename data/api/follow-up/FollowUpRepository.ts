...
export class FollowUpRepository extends BaseRepository implements IFollowUpRepository {
  ...
  async getById(id: string): Promise<Either<DataError, { task: FollowUpTask; attempts: FollowUpAttempt[] }>> { try { const { data } = await this.axios.get<{ task: FollowUpTask; attempts: FollowUpAttempt[] }>(`/api/follow-ups/${id}`); return Either.right(data) } catch (error) { return Either.left(mapToDataError(error)) } }
  ...
  async recordAttempt(id: string, input: RecordFollowUpAttemptInput): Promise<Either<DataError, { task: FollowUpTask; attempt: FollowUpAttempt }>> { try { const { data } = await this.axios.post<{ task: FollowUpTask; attempt: FollowUpAttempt }>(`/api/follow-ups/${id}/attempts`, input); return Either.right(data) } catch (error) { return Either.left(mapToDataError(error)) } }
